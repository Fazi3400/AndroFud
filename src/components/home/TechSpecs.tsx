...
export function TechSpecs({ brand, gradient, textColor }: TechSpecsProps) {
  ...
  return (
    <section
      className={`py-32 px-4 sm:px-6 lg:px-8 ${brand === "windowstools" ? "bg-gradient-to-b from-blue-950/30 via-slate-900/20 to-blue-950/30" : "bg-gradient-to-b from-[#1e0b3e] to-[#0f2a3d]"}`}
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {specs.map((spec, idx) => (
            ...
            >
              ...
              {/* Category Header */}
              <h3
                className={`text-sm font-black uppercase tracking-widest mb-6 relative inline-block px-3 py-1 rounded-lg border-2 transition-all ${
                  brand === "androfud"
              ...
              <div className="space-y-4 mt-6">
                {spec.items.map((item, itemIdx) => (
                  ...
                  >
                    <div
                      className={`mt-1.5 w-2.5 h-2.5 rounded-full flex-shrink-0 group-hover/item:scale-150 transition-transform ${brand === "androfud" ? "bg-gradient-to-r from-purple-400 to-pink-400" : brand === "windowstools" ? "bg-gradient-to-r from-cyan-500 to-blue-500" : "bg-gradient-to-r from-cyan-400 to-blue-400"}`}
                    ></div>
                    <span
                      className={`text-sm font-mono leading-relaxed transition-colors ${brand === "androfud" ? "text-purple-200/90 group-hover/item:text-white" : brand === "windowstools" ? "text-blue-200/80 group-hover/item:text-cyan-100" : "text-cyan-200/90 group-hover/item:text-white"}`}
                    >
                      {item}
                    </span>
                  </div>
              ...
              <div className="absolute bottom-0 right-0 w-16 h-16 opacity-0 group-hover:opacity-15 transition-opacity duration-300">
                <div
                  className={`absolute bottom-2 right-2 w-8 h-8 border-b-2 border-r-2 ${brand === "androfud" ? "border-purple-400" : brand === "windowstools" ? "border-emerald-600" : "border-cyan-400"}`}
                ></div>
              </div>
            </div>
          ))}
        ...
        {/* Bottom Info */}
        <div
          className={`mt-20 p-8 rounded-2xl text-center code-flow border-2 backdrop-blur-sm transition-all duration-300 hover:shadow-lg ${
            brand === "androfud"
        ...
        >
          <p
            className={`text-sm font-mono transition-colors ${brand === "windowstools" ? "text-emerald-400" : textColor} opacity-90`}
          >
