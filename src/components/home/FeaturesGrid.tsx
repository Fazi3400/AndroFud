...
export function FeaturesGrid({ features, brand, gradient, textColor }: FeaturesGridProps) {
  return (
    <section className={`py-32 px-4 sm:px-6 lg:px-8 ${brand === "windowstools" ? "bg-gradient-to-b from-blue-950/30 via-slate-900/20 to-blue-950/30" : "bg-gradient-to-b from-[#000000] to-[#000000]"}`}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-24 slide-in-up">
          ...
          <div className={`text-xl transition-all ${brand === "windowstools" ? "text-cyan-400/80" : textColor} opacity-80 max-w-2xl mx-auto`}>
            ...
            return (
              ...
              >
                ...
                <div className="mb-6 relative z-10">
                  <div className={`p-4 rounded-xl w-fit transition-all ${
                    brand === "androfud"
                    ...
                    {IconComponent && (
                      <IconComponent className={`w-8 h-8 transition-colors ${brand === "androfud" ? "text-purple-300" : brand === "windowstools" ? "text-cyan-400" : "text-cyan-300"}`} />
                    )}
                  </div>
                </div>
                ...
                <div className="relative z-10 space-y-4">
                  <h3 className={`text-2xl font-black transition-all ${brand === "windowstools" ? "text-cyan-400 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-cyan-300 group-hover:to-blue-300" : `${brand === "androfud" ? "text-white" : "text-white"} group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r ${brand === "androfud" ? "group-hover:from-purple-300 group-hover:to-pink-300" : "group-hover:from-cyan-300 group-hover:to-blue-300"}`}`}>
                    {feature.title}
                  </h3>
                  <p className={`text-sm leading-relaxed transition-colors ${brand === "windowstools" ? "text-blue-200/80 group-hover:text-cyan-100" : `${brand === "androfud" ? "text-purple-200/80" : "text-cyan-200/80"} group-hover:text-white/90`}`}>
                    {feature.description}
                  </p>
                </div>
                ...
                <div className="absolute bottom-0 right-0 w-20 h-20 opacity-0 group-hover:opacity-20 transition-opacity duration-300">
                  <div className={`absolute bottom-2 right-2 w-10 h-10 border-b-2 border-r-2 ${brand === "androfud" ? "border-purple-400" : brand === "windowstools" ? "border-cyan-500" : "border-cyan-400"}`}></div>
                </div>
              </div>
            );
          })}
        ...
        {/* Bottom Divider */}
        <div className={`border-t pt-12 text-center ${brand === "windowstools" ? "border-blue-600/40" : "border-[#0099ff]-500/10"}`}>
          <p className={`text-sm font-mono transition-colors ${brand === "windowstools" ? "text-cyan-500/60" : `${textColor} opacity-50`}`}>
