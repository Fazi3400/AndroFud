...
function validateFeedback(text: string): { blocked: boolean; reason?: string } {
  ...
  for (const keyword of FEEDBACK_BLOCKED_KEYWORDS) {
    if (lowerText.includes(keyword)) {
      return {
        blocked: true,
        reason: `Feedback contains restricted content: "${keyword}"`,
      };
    }
  }
  ...
  if (emailPattern.test(lowerText)) {
    return {
      blocked: true,
      reason: "Email addresses are not allowed in feedback",
    };
  }
  ...
  if (phonePattern.test(lowerText)) {
    return {
      blocked: true,
      reason: "Phone numbers are not allowed in feedback",
    };
  }
  ...
  if (repeatedChars && repeatedChars.length > 0) {
    return {
      blocked: true,
      reason: "Feedback contains suspicious repeated characters",
    };
  }
  ...
  if (specialChars > 5) {
    return {
      blocked: true,
      reason: "Feedback contains too many special characters",
    };
  }
  ...
  try {
    ...
    if (feedbackValidation.blocked) {
      return NextResponse.json(
        {
          error:
            feedbackValidation.reason || "Feedback contains restricted content",
        },
        { status: 400 },
      );
    }
